...
4. Выполните [миграции](https://knexjs.org/#Migrations). Для этого в командной строке (терминале) в папке server выполните:

    ```
    npm run migrate:run
    ```

    Одна из миграций заполняет базу демо-данными. Проверьте базу данных на их наличие.
