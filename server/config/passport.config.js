...
passport.use(
    ...
    new LocalStrategy({ usernameField: 'email' }, async (email, password, done) => {
        try {
            ...
            return isValid ? done(null, user) : done({ status: 401, message: 'Passwords do not match.' }, null, false);
        } catch (err) {
            return done(err);
        }
    })
...
passport.use(new JwtStrategy(options, async ({ id }, done) => {
    try {
        ...
        return user ? done(null, user) : done({ status: 401, message: 'Token is invalid.' }, false);
    } catch (err) {
        return done(err);
    }
}));
